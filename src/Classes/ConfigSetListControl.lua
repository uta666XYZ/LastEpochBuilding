...
-- Config Set list control — manages multiple named config presets
-- (e.g. "Boss", "Mapping", "Single Target"). Generic planner UI: it operates
-- purely on the ConfigTab's existing config-set backend (configSets /
-- configSetOrderList / NewConfigSet / SetActiveConfigSet) and contains no
-- game-specific content.
--
-- @leb-regression-guard:config-sets-ui
-- Config-set list control, adapted to LEB conventions from the PoB-parity
-- equivalent (see REGRESSION_GUARDS.md "config-sets-ui"):
--   * LEB ListControl / sub-control constructors take FLAT (anchor, x, y,
--     width, height, ...) args, NOT PoB's (anchor, rect, ...) table form.
--   * Dirty/rebuild flag is LEB's `configTab.build.modFlag` (the build owns
--     modFlag, set from tabs — see TreeTab), NOT PoB's `configTab.modFlag`
--     (which does not exist in LEB).
-- Backend (configSets etc.) was already inherited from PoB and is fully
-- functional in LEB; this is the missing UI on top of it.
--
...
function ConfigSetListClass:RenameSet(configSet, addOnName)
	...
	controls.save = new("ButtonControl", nil, -45, 70, 80, 20, "Save", function()
		...
		self.configTab.build.modFlag = true
		if addOnName then
